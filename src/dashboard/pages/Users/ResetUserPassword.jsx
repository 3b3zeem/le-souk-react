...
function ResetUserPassword({
  ...
  const validatePassword = () => {
    ...
    if (!passwordData.password.trim()) {
      ...
      newErrors.password =
        t("password_min_length") || "Password must be more than 6 characters";
    }

    if (!passwordData.confirmPassword.trim()) {
      newErrors.confirmPassword =
        t("confirm_password_required") || "Please confirm the password";
  ...
  const handlePasswordReset = async (e) => {
    ...
    try {
      ...
      if (!error.response?.data?.message) {
        toast.error(
          t("error_resetting_password") || "Error resetting password"
        );
      }
    } finally {
    ...
    <form onSubmit={handlePasswordReset}>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button
                  ...
                  } flex items-center text-gray-500 hover:text-gray-800`}
                  tabIndex={-1}
                >
      ...
      <div className="flex gap-3 mt-8 pt-6 border-t border-gray-200">
        <button
          ...
          onClick={onCancel}
          disabled={resettingPassword || disabled}
          className="flex-1 px-4 py-2 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
        >
          {t("cancel") || "Cancel"}
        </button>
        <button
          type="submit"
          disabled={resettingPassword || disabled}
          className="flex-1 flex items-center justify-center gap-2 px-6 py-2 bg-[#333E2C] text-white rounded-lg hover:bg-[#20281c] transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
        >
          {resettingPassword ? (
